...
const OUTPUT_PATH = path.resolve(DATA_DIR, "seed.sql");
...
async function run(): Promise<void> {
  ...
  statements.push("-- Auto-generated D1 seed SQL for heritage locations");
  statements.push("DROP TABLE IF EXISTS heritage_locations;");
  ...
  for (const item of rows) {
    ...
    statements.push(
      `INSERT INTO heritage_locations (id, name, longitude, latitude, category, era, deep_history, tags, tts_hints) VALUES (${sqlText(
        item.location_id
  ...
  console.log(`Generated ${OUTPUT_PATH} with ${insertedCount} INSERT rows.`);
}

run().catch((error) => {
  console.error("Failed to generate D1 seed SQL.");
